...
Проект изначально реализован на TypeScript. Компоненты, хранилище и утилитарные функции типизированы без использования `any` и инлайнового отключения линтера без необходимости.

## Проектная работа 5-го спринта

В приложении реализованы публичная лента заказов и история заказов пользователя с обновлением данных через WebSocket. Карточки показывают состав, стоимость и статус заказа, а подробная информация открывается в модальном окне или на отдельной странице при прямом переходе по URL.

## Проектная работа 6-го спринта

Все Redux reducers покрыты unit-тестами на Vitest. Пользовательские сценарии конструктора — drag and drop ингредиентов, просмотр деталей ингредиента и создание заказа — покрыты Playwright-тестами. HTTP API в Playwright-тестах полностью мокируется сохранённым HAR-файлом.

## Деплой

https://richbanker.github.io/react-burger/
